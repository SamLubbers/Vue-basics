<template lang="html">
  ...
  <template v-for='(action, index) in recordChronologicalOrder'>
  <div :key='index' class="alert" :class="action.attacker == 'player' ? 'alert-primary' : 'alert-danger'">
...
export default {
  ...
  methods: {
    ...
    botattack (maxForce) {
      let force = this.computeForce(maxForce) + 1
      this.playerHealth -= force
      this.recordAction('attack', 'bot', 'player', force)
    },
    playerAttack () {
      let force = this.computeForce(this.attackMaxforce)
      this.botHealth -= force
      this.recordAction('attack', 'player', 'bot', force)
      this.botattack(this.attackMaxforce)
    },
    playerSpecialAttack () {
      let force = this.computeForce(this.specialAttackMaxforce)
      this.botHealth -= force
      this.recordAction('attack', 'player', 'bot', force)
      this.botattack(this.attackMaxforce)
    },
    playerHeal () {
      let force = this.computeForce(this.playerMaxHeal)
      this.playerHealth += force
